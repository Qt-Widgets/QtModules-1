...
#include "qsysglobal.h"
/**
  * @单例模式的LOG
  */

#define tlog Singleton<TcLog>::instance()
...
public:
    ...
    void setLogLevel(int level = LOG_DEBUG);  //设置日志级别 LOG_DEBUG
    ...
    /// @param [in] SourceName 源文件名，建议用__FILE__作为参数。
    /// @param [in] SourceLine 源文件行，建议用__LINE__作为参数。
    inline void console(const QString& S,
            const QString& SourceName = QString(), 
            int SourceLine = 0)
    {
      #ifdef _DEBUG
